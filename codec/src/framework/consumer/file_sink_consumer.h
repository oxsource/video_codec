...
// Writes encoded packets to files: Annex-B video to `video_path`, ADTS audio to
// `audio_path` (if provided). Order and keyframe boundaries are preserved because
// packets arrive in order from the ring buffer. A muxer (`.mp4`) is just another
// PacketConsumer behind the same interface (deferred).
