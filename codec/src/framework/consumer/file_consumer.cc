...
FileConsumer::FileConsumer(std::string video_path, std::string audio_path) {
  if (!video_path.empty()) {
...
Status FileConsumer::Push(VideoPacket&& pkt) {
...
Status FileConsumer::Push(AudioPacket&& pkt) {
