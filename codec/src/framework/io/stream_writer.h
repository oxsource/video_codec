...
// ByteWriter that pushes bytes to a remote endpoint (cloud streaming / RTMP /
// SRT / HTTP upload) through caller-supplied write/flush callbacks. There is
// no separate transport interface: a network output is just a ByteWriter whose
// Seek/Tell are unsupported, so the source must be a fragmented (fMP4) muxer.
class StreamWriter : public ByteWriter {
 public:
  ...
  StreamWriter(WriteFn write, FlushFn flush)
