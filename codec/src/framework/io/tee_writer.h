...
// ByteWriter that fans every write/flush to N underlying writers (e.g. file +
...
class TeeWriter : public ByteWriter {
 public:
  explicit TeeWriter(std::vector<ByteWriter*> writers)
      : writers_(std::move(writers)) {}

  bool Write(const uint8_t* data, size_t size) override {
    for (ByteWriter* w : writers_) {
 ...
 private:
  std::vector<ByteWriter*> writers_;
};
