...
Status PixelConverter::Convert(const VideoFrame& src,
  ...
  if (to_nv12) {
    ...
    dst.stride[1] = w;
    dst.stride[2] = 0;
    libyuv::I420ToNV12(src.planes[0].data(), PlaneStride(src, 0),
                       src.planes[1].data(), PlaneStride(src, 1),
                       src.planes[2].data(), PlaneStride(src, 2),
    ...
    dst.stride[1] = cw;
    dst.stride[2] = cw;
    libyuv::NV12ToI420(src.planes[0].data(), PlaneStride(src, 0),
                       src.planes[1].data(), PlaneStride(src, 1),
