...
TEST(MediaFileFormatTest, HasExtensionExact) {
  ...
  EXPECT_TRUE(MediaFileFormat::HasExtension("/tmp/dir/clip.h264",
                                            MediaFileFormat::kH264));
  EXPECT_FALSE(MediaFileFormat::HasExtension("out.mp4", MediaFileFormat::kMp3));
  EXPECT_FALSE(
      MediaFileFormat::HasExtension("video.mp4x", MediaFileFormat::kMp4));
}

TEST(MediaFileFormatTest, HasExtensionIgnoresCase) {
  EXPECT_TRUE(MediaFileFormat::HasExtension("OUT.MP4", MediaFileFormat::kMp4));
  EXPECT_TRUE(
      MediaFileFormat::HasExtension("Clip.H264", MediaFileFormat::kH264));
}
...
TEST(MediaFileFormatTest, HasExtensionEdgeCases) {
  ...
  EXPECT_FALSE(
      MediaFileFormat::HasExtension("x.mp4.old", MediaFileFormat::kMp4));
}
