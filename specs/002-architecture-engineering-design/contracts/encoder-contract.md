...
4. **Output**: video `EncodedPacket::data` is **Annex-B** (SPS/PPS emitted at the first
   keyframe / on `Flush`); `keyframe` marks IDR frames. Audio `AudioPacket::data` is a
...
6. **Resource ownership**: `Encode` does not take ownership of `frame`/`buf`; the caller
   keeps them alive until the call returns. Returned `EncodedPacket` is owned by the
   caller.
