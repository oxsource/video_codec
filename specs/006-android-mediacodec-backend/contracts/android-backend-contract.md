...
- **C-013**: 输出为 Annex-B：`data` 含起始码；关键帧包前置 SPS/PPS（自 `BUFFER_FLAG_CODEC_CONFIG` 缓冲提取缓存）；`keyframe` 由 `BUFFER_FLAG_KEY_FRAME` 决定。
...
- **C-032**: 视频轨 `AMediaFormat` 写 `csd-0`（SPS）/`csd-1`（PPS）；音频轨写 `csd-0`（ASC）；另含 mime/尺寸/fps/采样率/声道元数据。
